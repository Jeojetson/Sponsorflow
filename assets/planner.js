(() => {
  ...
  const STATUS = [
    ...
    { id: 'IN_PROGRESS', label: 'In progress', help: 'Actively being worked' },
    {
      id: 'BLOCKED',
      label: 'Blocked',
      help: 'Waiting on a decision or dependency',
    },
    {
      id: 'REVIEW',
      label: 'Review / test',
      help: 'Needs validation or approval',
    },
  ...
  const SOURCE_CONFIDENCE = [
    ...
    {
      id: 'OFFICIAL_NO_CURRENT_DEADLINE',
      label: 'Official · deadline not posted',
    },
  ...
  async function init() {
    ...
    restoreIdentity();
    state.view =
      window.SponsorFlowStorage.getItem('asmePlannerView') || 'table';
    if (state.view === 'calendar') state.view = 'board';
    state.calendarMonth =
      window.SponsorFlowStorage.getItem('asmePlannerCalendarMonth') ||
      state.calendarMonth;
    if (!API || !API.configured()) {
      showConnectionError(
        'SponsorFlow is not connected. Add the Apps Script web app URL to assets/config.js.',
      );
      return;
    }
    let sort;
    try {
      sort = JSON.parse(
        window.SponsorFlowStorage.getItem('asmeWorkSort') || 'null',
      );
    } catch (_) {}
    if (
      sort &&
      ...
      state.sortDirection = sort.direction === 'desc' ? 'desc' : 'asc';
    }
    await loadPlanner();
  }

  function populateStaticOptions() {
    $('#statusFilter').innerHTML += STATUS.map(
      (item) => `<option value="${item.id}">${item.label}</option>`,
    ).join('');
    $('#priorityFilter').innerHTML += PRIORITY.map(
      (item) => `<option value="${item.id}">${item.label}</option>`,
    ).join('');
    $('#taskType').innerHTML = TASK_TYPE.map(
      (item) => `<option value="${item.id}">${item.label}</option>`,
    ).join('');
    $('#taskStatus').innerHTML = STATUS.map(
      (item) => `<option value="${item.id}">${item.label}</option>`,
    ).join('');
    $('#taskPriority').innerHTML = PRIORITY.map(
      (item) => `<option value="${item.id}">${item.label}</option>`,
    ).join('');
    $('#taskOrderStatus').innerHTML = ORDER_STATUS.map(
      (item) => `<option value="${item.id}">${item.label}</option>`,
    ).join('');
    $('#taskSourceConfidence').innerHTML = SOURCE_CONFIDENCE.map(
      (item) => `<option value="${item.id}">${item.label}</option>`,
    ).join('');
    $('#eventPriority').innerHTML = PRIORITY.map(
      (item) => `<option value="${item.id}">${item.label}</option>`,
    ).join('');
  }
  ...
  function bindEvents() {
    $('#projectNavigation').addEventListener('click', (event) => {
      ...
      window.SponsorFlowStorage.setItem(
        'asmePlannerBoard',
    ...
    $('#savePlannerNameButton').addEventListener('click', () =>
      saveIdentity($('#plannerActorName').value),
    );
    $('#plannerActorName').addEventListener('keydown', (event) => {
      if (event.key === 'Enter') {
        event.preventDefault();
        saveIdentity(event.currentTarget.value);
      }
    });
    $('#identityDialogSave').addEventListener('click', () => {
      if (saveIdentity($('#identityDialogName').value, true))
        $('#identityDialog').close();
    });
    $('#identityDialogName').addEventListener('keydown', (event) => {
      if (event.key === 'Enter') {
        event.preventDefault();
        $('#identityDialogSave').click();
      }
    });
    ...
    $$('[data-open-workspace]').forEach((button) =>
      button.addEventListener('click', openWorkspaceDialog),
    );
    ...
    $('#downloadCalendarButton').addEventListener(
      'click',
      downloadBoardCalendar,
    );
    $('#calendarSubscriptionsButton').addEventListener(
      'click',
      openCalendarSubscriptions,
    );
    $('#openCalendarSubscriptionsButton').addEventListener(
      'click',
      openCalendarSubscriptions,
    );
    $('#addCalendarEventButton').addEventListener('click', () =>
      openEventDialog('', { startDate: todayText(), endDate: todayText() }),
    );
    $('#calendarPreviousButton').addEventListener('click', () =>
      moveCalendarMonth(-1),
    );
    $('#calendarTodayButton').addEventListener('click', () => {
      state.calendarMonth = `${todayText().slice(0, 7)}-01`;
      renderCalendar();
    });
    ...
    [
      ...
      $('#' + id).addEventListener(
        id.endsWith('Filter') && !['taskSearch'].includes(id)
          ? 'change'
          : 'input',
        renderCurrentBoard,
      );
    ...
    $('#insightsScope').addEventListener('change', (event) => {
      state.insightsScope = event.currentTarget.value;
      renderInsights();
    });
    ...
    $('#workspaceDirectoryList').addEventListener(
      'click',
      handleWorkspaceDirectoryClick,
    );

    $('#taskForm').addEventListener('submit', saveTask);
    $('#taskBoardId').addEventListener('change', () =>
      renderDependencyPicker($('#taskId').value, selectedDependencies()),
    );
    $('#archiveTaskButton').addEventListener('click', archiveTask);
    $('#downloadTaskCalendarButton').addEventListener(
      'click',
      downloadTaskCalendarFromForm,
    );
    ...
    $$('dialog.modal').forEach((dialog) => {
      ...
      dialog.addEventListener('cancel', (event) => {
        if (
          (dialog.id === 'taskDialog' && state.taskDirty) ||
          (dialog.id === 'eventDialog' && state.eventDirty)
        ) {
          event.preventDefault();
  ...
  function saveIdentity(value, fromDialog = false) {
    ...
    if (state.afterIdentity) {
      const next = state.afterIdentity;
  ...
  function requireActor() {
    ...
    $('#identityDialogStatus').textContent =
      'Enter your name before making changes.';
  ...
  async function loadPlanner(options = {}) {
    ...
    const snapshot = API.cached?.('plannerBootstrap');
    if (snapshot && !state.tasks.length) {
      applyPlannerBootstrap(snapshot.data, previousBoardId);
      setPlannerView(previousView);
      renderCurrentBoard();
    }
    setLoading(!snapshot && !state.tasks.length);
    $('#plannerSyncStatus').textContent = snapshot
      ? 'Showing saved work · checking for updates…'
      : 'Connecting to the team workspace…';
    try {
      ...
      renderCurrentBoard();
      hideConnectionError();
      $('#plannerSyncStatus').textContent =
        'Updated ' +
        new Date(
          API.cached?.('plannerBootstrap')?.savedAt || Date.now(),
      ...
      $('#plannerSyncStatus').textContent = snapshot
        ? 'Saved work · updates unavailable'
        : 'Could not connect';
  ...
  function applyPlannerBootstrap(data, preferredBoardId = '') {
    ...
    state.tasks = (Array.isArray(data.tasks) ? data.tasks : []).map(
      normalizePlannerTaskDates,
    );
  ...
  function chooseInitialBoard(preferredId = '') {
    ...
    const fallback = state.teams.some((team) => team.id === CLUB_TEAM_ID)
      ? AGGREGATE_BOARD_ID
      : state.boards[0]?.id || '';
    ...
    if (candidate === AGGREGATE_BOARD_ID) {
      ...
      const selected =
        state.boards.find((board) => board.id === candidate) ||
        state.boards[0] ||
        null;
  ...
  function populateTaskBoardOptions(selectedId = '') {
    const grouped = state.teams
      ...
      .join('');
    $('#taskBoardId').innerHTML =
      grouped || '<option value="">Create a project first</option>';
    if (selectedId && state.boards.some((board) => board.id === selectedId))
      $('#taskBoardId').value = selectedId;
  }
  ...
  function populateEventBoardOptions(selectedId = '') {
    const grouped = state.teams
      ...
      .join('');
    $('#eventBoardId').innerHTML =
      grouped || '<option value="">Create a project first</option>';
    const candidate =
      selectedId && state.boards.some((board) => board.id === selectedId)
        ? selectedId
        : defaultTaskBoardId();
  ...
  function renderBoardOptions() {
    const filtered = state.boards.filter(
      (board) => !state.currentTeamId || board.teamId === state.currentTeamId,
    );
    const aggregate =
      state.currentTeamId === CLUB_TEAM_ID
  ...
  function handleTeamChange() {
    ...
    renderBoardOptions();
    const available = state.boards.filter(
      (board) => board.teamId === state.currentTeamId,
    );
    state.currentBoardId =
      state.currentTeamId === CLUB_TEAM_ID
        ? AGGREGATE_BOARD_ID
  ...
  function persistBoardSelection() {
    ...
    if (state.currentBoardId)
      url.searchParams.set('board', state.currentBoardId);
  ...
  function aggregateBoard() {
    ...
    return {
      ...
      teamId: CLUB_TEAM_ID,
      name: 'All work',
      description: 'Tasks, parts, and dates across active teams.',
      targetStart: dates[0] || '',
  ...
  function isAggregateBoard(board = currentBoard()) {
    return Boolean(board?.isAggregate || board?.id === AGGREGATE_BOARD_ID);
  }
  function currentBoard() {
    return state.currentBoardId === AGGREGATE_BOARD_ID
      ? aggregateBoard()
  ...
  function currentTeam() {
    ...
    return (
      state.teams.find(
        (team) => team.id === (board?.teamId || state.currentTeamId),
      ) || null
    );
  }
  function boardTasks() {
    return isAggregateBoard()
      ? state.tasks.filter(
          (task) => !task.archived && teamForTask(task)?.id !== 'TEAM-OPS',
        )
      : state.tasks.filter(
          (task) => task.boardId === state.currentBoardId && !task.archived,
        );
  }
  function actualBoardForTask(task) {
    return state.boards.find((board) => board.id === task?.boardId) || null;
  }
  function teamForTask(task) {
    const board = actualBoardForTask(task);
    return state.teams.find((team) => team.id === board?.teamId) || null;
  }
  function defaultTaskBoardId() {
    if (
      !isAggregateBoard() &&
      state.boards.some((board) => board.id === state.currentBoardId)
    )
      return state.currentBoardId;
    return (
      state.boards.find((board) => board.teamId === CLUB_TEAM_ID)?.id ||
      state.boards[0]?.id ||
      ''
    );
  }
  ...
  function filteredTasks(source = boardTasks()) {
    ...
    return source.filter((task) => {
      ...
      if (
        state.quickFilter === 'mine' &&
        !splitList(task.ownerNames).some(
          (name) => normalizeKey(name) === normalizeKey(state.actorName),
        )
      )
        return false;
      if (
        state.quickFilter === 'week' &&
        !(
          task.status !== 'DONE' &&
          task.dueDate &&
          task.dueDate >= today &&
          daysBetween(today, task.dueDate) <= 7
        )
      )
        return false;
      if (
        ...
        !(
          task.status !== 'DONE' &&
          (task.status === 'BLOCKED' || (task.dueDate && task.dueDate < today))
        )
      )
        return false;
      const haystack = [
      ...
      if (
        owner &&
        !splitList(task.ownerNames).some(
          (name) => normalizeKey(name) === normalizeKey(owner),
        )
      )
        return false;
  ...
  function renderCurrentBoard() {
    ...
    $('#boardTeamBadge').textContent = isAggregateBoard(board)
      ? 'ALL · CLUB-WIDE'
      : `${team?.icon || 'TEAM'} · ${team?.name || 'Team'}`;
    $('#boardTitle').textContent = board.name;
    $('#boardDescription').textContent =
      board.description || 'Shared team project.';
    $('#boardDateRange').textContent = formatBoardDateRange(board);
    $('#editBoardButton').classList.toggle(
      'is-hidden',
      isAggregateBoard(board),
    );
    $('#editBoardButton').disabled = false;
    $('#editBoardButton').title = 'Edit this project';
    renderOwnerFilter();
    const count = filteredTasks().length;
    $('#plannerResultCount').textContent =
      `${count} of ${boardTasks().length} items`;
    const filterCount =
      ['statusFilter', 'priorityFilter', 'ownerFilter'].filter(
        (id) => $('#' + id).value,
      ).length +
      ['partsOnlyFilter', 'hideDoneFilter'].filter((id) => $('#' + id).checked)
        .length;
  ...
  function renderOwnerFilter() {
    ...
    const owners = Array.from(
      new Set(boardTasks().flatMap((task) => splitList(task.ownerNames))),
  ...
  function renderMetrics() {
    ...
    const dueSoon = tasks.filter(
      (task) =>
        ...
        task.dueDate &&
        task.dueDate >= now &&
        daysBetween(now, task.dueDate) <= 7,
    ).length;
    const awaitingOrder = tasks.filter((task) =>
      ['NEEDS_SPEC', 'NEEDS_QUOTE', 'READY_TO_ORDER'].includes(
        task.orderStatus,
      ),
    ).length;
    ...
    const knownFunding = funding.reduce(
      (sum, task) => sum + (Number(task.fundingMax) || 0),
      0,
    );
    const variableFunding = funding.filter(
      (task) => !Number(task.fundingMax),
    ).length;
    ...
    const fourth = funding.length
      ? {
          ...
          value: knownFunding ? formatCurrency(knownFunding) : funding.length,
          detail: `${funding.length} ${funding.length === 1 ? 'opportunity' : 'opportunities'}${variableFunding ? ` · ${variableFunding} variable` : ''}`,
          tone: 'funding',
        }
      : {
          label: 'Parts to source',
          value: awaitingOrder,
          detail: `${formatCurrency(estimatedCost)} estimated total`,
          tone: awaitingOrder ? 'parts' : 'neutral',
        };
    const metrics = [
      {
        label: 'Overall progress',
        value: `${completion}%`,
        detail: `${done} of ${tasks.length} tasks done`,
        tone: 'completion',
      },
      {
        ...
        value: tasks.filter(
          (task) =>
            task.status !== 'DONE' &&
            (task.status === 'BLOCKED' || (task.dueDate && task.dueDate < now)),
        ).length,
        detail: `${overdue} overdue · ${blocked} blocked`,
        tone: overdue + blocked ? 'danger' : 'good',
      },
      {
        label: 'Due in 7 days',
        value: dueSoon,
        detail: 'Open work due soon',
        tone: dueSoon ? 'warning' : 'neutral',
      },
      fourth,
    ];
    $('#plannerMetrics').innerHTML = metrics
      .map(
        (metric) => `
  ...
  function renderKanban() {
    ...
    $$('.planner-task-card').forEach((card) => {
      card.addEventListener('click', (event) => {
        if (!event.defaultPrevented) openTaskDialog(card.dataset.taskId);
      });
    ...
    $$('[data-drop-status]').forEach((column) => {
      ...
      column.addEventListener('drop', async (event) => {
        ...
        column.classList.remove('is-drag-over');
        const taskId =
          event.dataTransfer.getData('text/plain') || state.draggedTaskId;
        await moveTask(taskId, column.dataset.dropStatus);
      });
    });
    $$('[data-quick-add-status]').forEach((button) =>
      button.addEventListener('click', () =>
        openTaskDialog('', { status: button.dataset.quickAddStatus }),
      ),
    );
  }
  ...
  function renderGantt() {
    ...
    const tasks = filteredTasks()
      ...
      .sort(
        (a, b) =>
          (a.startDate || a.dueDate || '').localeCompare(
    ...
    if (!tasks.length) {
      ...
      host
        .querySelector('[data-empty-add-task]')
    ...
    const rows = tasks
      .map((task) => {
        ...
        const width = Math.max(
          task.isMilestone ? 16 : dayWidth * durationDays,
          12,
        );
        const owners = splitList(task.ownerNames).join(', ') || 'Unassigned';
        const overdue =
          task.status !== 'DONE' && task.dueDate && task.dueDate < todayText();
        const dependencyNames = splitList(task.dependencyIds)
    ...
    host
      .querySelectorAll('[data-open-task]')
      .forEach((button) =>
        button.addEventListener('click', () =>
  ...
  function buildTimelineAxis(startText, totalDays, dayWidth) {
    ...
    while (index < totalDays) {
      ...
      const remainingInMonth =
        daysInMonth(day.getFullYear(), day.getMonth()) - day.getDate() + 1;
      const span = Math.min(remainingInMonth, totalDays - index);
      segments.push(
        `<div class="gantt-month" style="left:${index * dayWidth}px;width:${span * dayWidth}px"><strong>${day.toLocaleDateString('en-US', { month: 'short', year: 'numeric' })}</strong></div>`,
      );
      index += span;
    }
    const weeks = [];
    for (let dayIndex = 0; dayIndex < totalDays; dayIndex += 7) {
      ...
      weeks.push(
        `<div class="gantt-week" style="left:${dayIndex * dayWidth}px;width:${Math.min(7, totalDays - dayIndex) * dayWidth}px"><span>${formatAxisDate(date)}</span></div>`,
      );
    }
  ...
  function renderTaskTable() {
    const tasks = filteredTasks().sort(listSort);
    $('#workSort').value = state.sortField;
    $('#workSortDirection').textContent =
      state.sortDirection === 'asc' ? 'Ascending ↑' : 'Descending ↓';
    const host = $('#workList');
    if (!tasks.length) {
      host.innerHTML =
        '<div class="work-list-empty"><strong>No work in this view</strong><p>Try another project or clear your filters.</p><button type="button" class="button button-secondary button-small" data-clear-work>Clear filters</button></div>';
      host
        .querySelector('[data-clear-work]')
    ...
    host.innerHTML =
      ...
      [
        ...
        .map(
          ([field, label]) =>
            `<button type="button" data-sort="${field}" aria-label="Sort ${label}, ${state.sortField === field && state.sortDirection === 'asc' ? 'descending' : 'ascending'}">${label}${state.sortField === field ? (state.sortDirection === 'asc' ? ' ↑' : ' ↓') : ' ↕'}</button>`,
        )
        .join('') +
      '</div>' +
      tasks
        ...
        <span class="work-row-title"><strong>${escapeHtml(task.title)}</strong><small>${context.map((label) => `<span class="work-meta">${escapeHtml(label)}</span>`).join('')}</small></span>
    ...
    host
      .querySelectorAll('[data-table-task]')
      .forEach((row) =>
        row.addEventListener('click', () =>
          openTaskDialog(row.dataset.tableTask),
        ),
      );
  }
  ...
  function renderProjectNavigation() {
    ...
    const closed = new Set(
      Array.from(host.querySelectorAll('details:not([open])')).map(
        (item) => item.dataset.team,
      ),
    );
    const item = (id, title, count) =>
      `<button type="button" class="sidebar-item${state.currentBoardId === id ? ' is-active' : ''}" data-project-id="${escapeHtml(id)}"${state.currentBoardId === id ? ' aria-current="true"' : ''}><span>${escapeHtml(title)}</span><small>${count}</small></button>`;
    const activeTasks = state.tasks.filter((task) => !task.archived);
    host.innerHTML =
      item(
        AGGREGATE_BOARD_ID,
        'All work',
        activeTasks.filter((task) => teamForTask(task)?.id !== 'TEAM-OPS')
          .length,
      ) +
      '<p class="sidebar-label">Teams & projects</p>' +
      state.teams
        .filter((team) => team.id !== 'TEAM-OPS')
        .map((team) => {
        ...
        .join('') +
      '<details class="legacy-work"><summary>Finance records</summary>' +
      state.boards
        .filter((board) => board.teamId === 'TEAM-OPS')
        .map((board) =>
          item(
            board.id,
            board.name,
            activeTasks.filter((task) => task.boardId === board.id).length,
          ),
        )
        .join('') +
      '</details>';
  ...
  function renderCalendar() {
    ...
    $('#calendarMonthTitle').textContent = monthStart.toLocaleDateString(
      'en-US',
    ...
    for (let index = 0; index < 42; index += 1) {
      ...
      const dayTasks = tasks.filter((task) =>
        calendarTaskTouchesDate(task, value),
      );
    ...
    grid.querySelectorAll('[data-calendar-task]').forEach((button) =>
      button.addEventListener('click', () => {
        const task = state.tasks.find(
          (item) => item.id === button.dataset.calendarTask,
        );
        if (task?.taskType === 'MEETING') openEventDialog(task.id);
        else openTaskDialog(button.dataset.calendarTask);
      }),
    );
    grid.querySelectorAll('[data-calendar-add]').forEach((button) =>
      button.addEventListener('click', (event) => {
        event.stopPropagation();
        openEventDialog('', {
          startDate: button.dataset.calendarAdd,
          endDate: button.dataset.calendarAdd,
        });
      }),
    );
    grid
      .querySelectorAll('[data-calendar-date-focus]')
      .forEach((button) =>
        button.addEventListener('click', () =>
          focusCalendarAgendaDate(button.dataset.calendarDateFocus),
        ),
      );

    const monthTasks = tasks.filter((task) =>
      calendarTaskIntersectsRange(task, monthStartText, monthEndText),
    );
    ...
    agenda.querySelectorAll('[data-calendar-agenda-task]').forEach((button) =>
      button.addEventListener('click', () => {
        const task = state.tasks.find(
          (item) => item.id === button.dataset.calendarAgendaTask,
        );
        if (task?.taskType === 'MEETING') openEventDialog(task.id);
        else openTaskDialog(button.dataset.calendarAgendaTask);
      }),
    );
    agenda
      .querySelectorAll('[data-calendar-agenda-download]')
      .forEach((button) =>
        button.addEventListener('click', (event) => {
          event.stopPropagation();
          const task = state.tasks.find(
            (item) => item.id === button.dataset.calendarAgendaDownload,
          );
  ...
  function focusCalendarAgendaDate(value) {
    const task = filteredTasks().find(
      (item) =>
        ...
        calendarTaskTouchesDate(item, value),
    );
    const item = task
      ? $('#calendarAgenda')?.querySelector(
          `[data-agenda-task-id="${CSS.escape(task.id)}"]`,
        )
      : null;
    if (!item) return;
  ...
  function setPlannerView(view) {
    ...
    state.view = ['board', 'timeline', 'table', 'insights'].includes(view)
      ? view
      : 'board';
    ...
    $$('[data-planner-panel]').forEach((panel) =>
      panel.classList.toggle(
        'is-hidden',
        panel.dataset.plannerPanel !== state.view,
      ),
    );
  ...
  async function moveTask(taskId, status) {
    if (
      !taskId ||
      !STATUS.some((item) => item.id === status) ||
      !requireActor()
    )
      return;
    ...
    try {
      ...
      replaceTask(saved);
      setPlannerMessage(
        `${task.title} moved to ${statusLabel(status)}.`,
        'success',
      );
      renderCurrentBoard();
    } catch (error) {
  ...
  function editCurrentBoard() {
    ...
    if (isAggregateBoard(board)) {
      setPlannerMessage(
        'The Club-wide Portfolio is generated automatically from every team timeline. Edit an individual team timeline instead.',
        'error',
      );
      return;
    }
  ...
  async function saveTeam(event) {
    ...
    try {
      ...
      if (index >= 0) state.teams[index] = saved;
      else state.teams.push(saved);
  ...
  async function saveBoard(event) {
    ...
    try {
      ...
      if (index >= 0) state.boards[index] = saved;
      else state.boards.push(saved);
  ...
  function renderWorkspaceDirectory() {
    ...
    host.innerHTML =
      state.teams
        ...
        .join('') ||
      '<div class="workspace-no-board">Create the first team.</div>';
  }
  ...
  function handleWorkspaceDirectoryClick(event) {
    ...
    if (teamButton)
      fillTeamForm(
        state.teams.find((item) => item.id === teamButton.dataset.editTeam),
      );
    if (boardButton)
      fillBoardForm(
        state.boards.find((item) => item.id === boardButton.dataset.editBoard),
      );
  }

  async function openTaskDialog(taskId = '', defaults = {}) {
    if (!state.actorName) {
      state.afterIdentity = () => openTaskDialog(taskId, defaults);
      requireActor();
      return;
    }
    if (!currentBoard()) {
    ...
    if (!taskId) {
      ...
      $('#taskOrderStatus').value =
        defaults.taskType === 'PURCHASE' ? 'NEEDS_SPEC' : 'NOT_NEEDED';
  ...
  function fillTaskForm(task) {
    state.suppressDirty = true;
    $('#taskDialogEyebrow').textContent =
      `${teamForTask(task)?.name || 'Team'} · ${statusLabel(task.status)}`;
    ...
    $('#taskFundingMin').value =
      task.fundingMin === '' || task.fundingMin == null ? '' : task.fundingMin;
    $('#taskFundingMax').value =
      task.fundingMax === '' || task.fundingMax == null ? '' : task.fundingMax;
    ...
    $('#taskUpdatedMeta').textContent =
      `Last updated ${relativeTime(task.updatedAt)} by ${task.updatedBy || 'unknown'}`;
  ...
  function setTaskTab(tab) {
    const selected = [
      ...
      : 'overview';
    $$('[data-task-tab]').forEach((button) =>
      button.classList.toggle('is-active', button.dataset.taskTab === selected),
    );
    $$('[data-task-panel]').forEach((panel) =>
      panel.classList.toggle('is-hidden', panel.dataset.taskPanel !== selected),
    );
  }
  ...
  function renderDependencyPicker(currentTaskId, selectedIds = []) {
    ...
    const missing = selectedIds.filter(
      (id) => !candidates.some((task) => task.id === id),
    );
    $('#taskDependencyList').innerHTML = candidates.length
      ? candidates
          .map(
            (task) => `
      <label class="dependency-option">
        ...
        <span><strong>${escapeHtml(task.title)}</strong><small>${statusLabel(task.status)} · ${priorityLabel(task.priority)}</small></span>
          ...
          .join('')
      : '<p class="dependency-empty">No other active tasks on this project.</p>';
    $('#taskDependencyList').insertAdjacentHTML(
      'beforeend',
      missing
        .map(
          (id) =>
            `<input type="checkbox" hidden checked value="${escapeHtml(id)}">`,
        )
        .join(''),
    );
  }
  ...
  function selectedDependencies() {
    return Array.from(
      $('#taskDependencyList').querySelectorAll(
  ...
  async function saveTask(event) {
    ...
    if (payload.title.length < 2) {
      setTaskStatus('Give the task a clear title.', 'error');
      setTaskTab('overview');
      $('#taskTitle').focus();
      return;
    }
    ...
    if (
      ...
      setTaskStatus(
        'For a same-day event, the end time must be after the start time.',
        'error',
      );
      setTaskTab('schedule');
      return;
    ...
    if (
      ...
      setTaskStatus('Progress must be between 0 and 100.', 'error');
      setTaskTab('overview');
      return;
    }
    ...
    try {
      ...
      renderCurrentBoard();
      setPlannerMessage(
        `${saved.title} saved and verified in the shared planner.`,
        'success',
      );
    } catch (error) {
      setTaskStatus(error.message, 'error');
      if (/someone else|refresh/i.test(error.message)) {
        setTaskStatus(
          `${error.message} Your unsaved draft is still stored in this browser.`,
          'error',
        );
      }
    } finally {
  ...
  async function archiveTask() {
    ...
    if (
      !window.confirm(
        `Archive “${task.title}”? It will leave active views but remain in the Google Sheet and activity history.`,
      )
    )
      return;
    setTaskStatus('Archiving…');
    try {
      await API.post('archivePlannerTask', {
        taskId: id,
        actorName: state.actorName,
        expectedUpdatedAt: task.updatedAt || '',
      });
  ...
  async function addTaskComment() {
    ...
    if (!taskId || body.length < 2) {
      setTaskStatus('Write a comment before posting.', 'error');
      return;
    }
    $('#addTaskCommentButton').disabled = true;
    try {
      const result = await API.post('addPlannerComment', {
        taskId,
        body,
        actorName: state.actorName,
      });
      ...
      $('#addTaskCommentButton').disabled = false;
    }
  }
  ...
  function renderTaskConversation(detail) {
    ...
    $('#taskComments').innerHTML = comments.length
      ...
      <article class="task-comment"><div class="comment-avatar">${escapeHtml(initials(comment.authorName))}</div><div><header><strong>${escapeHtml(comment.authorName)}</strong><time>${escapeHtml(relativeTime(comment.createdAt))}</time></header><p>${escapeHtml(comment.body).replace(/\n/g, '<br>')}</p></div></article>`,
          ...
          .join('')
      : '<p class="comment-empty">No comments yet. Use this space for decisions, blockers, and handoffs.</p>';
    $('#taskActivitySection').classList.toggle('is-hidden', !activity.length);
    $('#taskActivity').innerHTML = activity
      .slice(0, 20)
      .map(
        (item) =>
          `<li><span></span><div><strong>${escapeHtml(activityLabel(item.action))}</strong><p>${escapeHtml(item.details || '')}</p><small>${escapeHtml(item.actor || 'Unknown')} · ${escapeHtml(relativeTime(item.createdAt))}</small></div></li>`,
      )
      .join('');
  }
  ...
  function openEventDialog(taskId = '', defaults = {}) {
    ...
    if (task) {
      ...
      $('#eventStartDate').value = normalizeDateInput(
        task.startDate || task.dueDate,
      );
      $('#eventEndDate').value = normalizeDateInput(
      ...
      $('#eventEndDate').value =
        defaults.endDate || defaults.startDate || todayText();
  ...
  async function saveCalendarEvent(event) {
    ...
    if (title.length < 2) {
      setEventStatus('Give the event a clear title.', 'error');
      return;
    }
    if (!startDate) {
      setEventStatus('Choose a date.', 'error');
      return;
    }
    ...
    if (
      ...
      setEventStatus('The end time must be after the start time.', 'error');
      return;
    }
    ...
    try {
      ...
      renderCalendar();
      setPlannerMessage(
        `${saved.title} saved to the shared calendar.`,
        'success',
      );
    } catch (error) {
      setEventStatus(
        `${error.message} Your unsaved event is still stored in this browser.`,
        'error',
      );
    } finally {
  ...
  function requestDialogClose(dialogId) {
    ...
    if (
      dirty &&
      !window.confirm(
        'Discard unsaved changes? A local draft will remain available on this browser.',
      )
    )
      return;
    dialog.close();
  ...
  function saveTaskDraft() {
    try {
      window.SponsorFlowStorage.setItem(
        taskDraftKey(),
        JSON.stringify({ savedAt: Date.now(), payload: buildTaskPayload() }),
      );
    } catch (_) {}
  }
  ...
  function restoreTaskDraft(task = null) {
    try {
      const raw = window.SponsorFlowStorage.getItem(
        taskDraftKey(
          task?.id || 'new',
          task?.boardId || $('#taskBoardId').value,
        ),
      );
      if (!raw) return;
      const draft = JSON.parse(raw);
      const serverTime = task?.updatedAt
        ? new Date(task.updatedAt).getTime()
        : 0;
      if (!draft?.payload || Number(draft.savedAt || 0) <= serverTime) return;
      if (!window.confirm('Restore your unsaved task draft from this browser?'))
        return;
      applyTaskDraft(draft.payload);
      state.taskDirty = true;
      setTaskStatus(
        'Unsaved draft restored. Save to publish it to the shared planner.',
        'success',
      );
    } catch (_) {}
  }
  ...
  function applyTaskDraft(draft) {
    ...
    const map = {
      ...
      tags: 'taskTags',
      campus: 'taskCampus',
      fundingMin: 'taskFundingMin',
      fundingMax: 'taskFundingMax',
      fundingAmountLabel: 'taskFundingAmountLabel',
      sourceUrl: 'taskSourceUrl',
      sourceConfidence: 'taskSourceConfidence',
      requirements: 'taskRequirements',
      partName: 'taskPartName',
      partNumber: 'taskPartNumber',
      vendor: 'taskVendor',
    ...
    if (draft.boardId) {
      populateTaskBoardOptions(draft.boardId);
      $('#taskBoardId').value = draft.boardId;
    }
  ...
  function restoreEventDraft(task = null) {
    try {
      const raw = window.SponsorFlowStorage.getItem(
        eventDraftKey(
          task?.id || 'new',
          task?.boardId || $('#eventBoardId').value,
        ),
      );
      if (!raw) return;
      const draft = JSON.parse(raw);
      ...
      state.suppressDirty = true;
      if (d.boardId) {
        populateEventBoardOptions(d.boardId);
        $('#eventBoardId').value = d.boardId;
      }
      $('#eventTitle').value = d.title || '';
      $('#eventStartDate').value = d.startDate || '';
      $('#eventEndDate').value = d.endDate || d.startDate || '';
      $('#eventAllDay').checked = d.allDay !== false;
      $('#eventStartTime').value = d.startTime || '';
      $('#eventEndTime').value = d.endTime || '';
      $('#eventLocation').value = d.location || '';
      $('#eventOwners').value = d.ownerNames || '';
      $('#eventPriority').value = d.priority || 'MEDIUM';
      $('#eventDescription').value = d.description || '';
      $('#eventImportantDate').checked = Boolean(d.importantDate);
      updateEventTimeFields();
      state.suppressDirty = false;
      state.eventDirty = true;
      setEventStatus(
        'Unsaved event draft restored. Save to publish it.',
        'success',
      );
    } catch (_) {}
  }

  function clearTaskDraft(id, boardId) {
    try {
      window.SponsorFlowStorage.removeItem(taskDraftKey(id, boardId));
    } catch (_) {}
  }
  function clearEventDraft(id, boardId) {
    try {
      window.SponsorFlowStorage.removeItem(eventDraftKey(id, boardId));
    } catch (_) {}
  }
  ...
  function updateTaskCalendarButton() {
    ...
    const hasDate = Boolean(
      $('#taskStartDate').value || $('#taskDueDate').value,
    );
  ...
  function downloadTaskCalendarFromForm() {
    ...
    if (!task.startDate && !task.dueDate) {
      setTaskStatus(
        'Add a start date or due date before downloading a calendar event.',
        'error',
      );
      return;
    }
  ...
  function downloadBoardCalendar() {
    ...
    if (!tasks.length) {
      setPlannerMessage(
        'This project has no dated tasks to add to a calendar.',
        'error',
      );
      return;
    }
    ...
    setPlannerMessage(
      `${tasks.length} dated ${tasks.length === 1 ? 'task' : 'tasks'} exported as an .ics calendar.`,
      'success',
    );
  }
  ...
  function downloadTasksCalendar(tasks, name) {
    ...
    const events = tasks
      ...
      .map((task) => {
        ...
        const description = [
          ...
          task.requirements
            ? `Requirements / next action: ${task.requirements}`
            : '',
          task.sourceUrl ? `Official source: ${task.sourceUrl}` : '',
        ]
          .filter(Boolean)
          .join('\n');
        const uidBase = String(task.id || `${task.title}-${start}`).replace(
          /[^a-zA-Z0-9._-]/g,
          '-',
        );
        return [
    ...
    downloadTextFile(
      `${slugify(name || selectedBoard?.name || 'asme-project-calendar')}.ics`,
      calendar,
  ...
  function openCalendarSubscriptions() {
    if (!state.calendarFeedBaseUrl) {
      setPlannerMessage(
        'Redeploy the v8 Apps Script backend before creating live calendar subscriptions.',
        'error',
      );
      return;
    }
  ...
  function renderCalendarSubscriptions() {
    const items = [
      {
        ...
        badge: 'Recommended',
        description:
          'Every dated task across every team, including Finance & Sponsorship opportunities and deadlines.',
      },
      {
        key: 'important',
        scope: 'important',
        ...
        badge: 'Recommended',
        description:
          'Races, competitions, meetings, milestones, inspections, and critical club deadlines. Funding opportunities are excluded.',
      },
      ...state.teams
        .filter((team) => team.id !== CLUB_TEAM_ID)
        .map((team) => ({
          key: `team-${team.id}`,
          scope: 'team',
          id: team.id,
          ...
          badge: 'Team calendar',
          description:
            team.description || `All dated work owned by ${team.name}.`,
        })),
    ];
    const board = currentBoard();
    if (board && !isAggregateBoard(board)) {
      items.push({
        ...
        description:
          board.description ||
          'Only dates from the timeline currently open in SponsorFlow.',
      });
    }

    $('#calendarSubscriptionList').innerHTML = items
      .map((item) => {
    ...
    host.querySelectorAll('[data-copy-subscription]').forEach((button) =>
      button.addEventListener('click', () => {
        const input = host.querySelector(
          `[data-subscription-input="${CSS.escape(button.dataset.copySubscription)}"]`,
        );
        if (input) copySubscriptionUrl(input.value);
      }),
    );
    host.querySelectorAll('[data-open-subscription]').forEach((button) =>
      button.addEventListener('click', () => {
        const input = host.querySelector(
          `[data-subscription-input="${CSS.escape(button.dataset.openSubscription)}"]`,
        );
        if (!input) return;
        window.location.href = input.value.replace(/^https:/i, 'webcal:');
        setSubscriptionStatus(
          'Your calendar app should open. If it does not, use Copy URL and add it manually.',
          'success',
        );
      }),
    );
  ...
  function shareBoard() {
    ...
    if (navigator.clipboard && window.isSecureContext) {
      navigator.clipboard
        .writeText(text)
        .then(() => setPlannerMessage('Board link copied.', 'success'))
        .catch(() => window.prompt('Copy this board link:', text));
  ...
  function exportBoardCsv() {
    ...
    const rows = boardTasks().map((task) => {
      ...
      return [
        ...
        task.fundingAmountLabel,
        task.sourceUrl,
        sourceConfidenceLabel(task.sourceConfidence),
        task.requirements,
        task.partName,
        task.partNumber,
        task.vendor,
        task.quantity,
        task.estimatedCost,
        orderLabel(task.orderStatus),
        splitList(task.dependencyIds)
  ...
  function setTaskStatus(message, tone = '') {
    const element = $('#taskFormStatus');
    element.textContent = message || '';
    element.className = `form-status${tone ? ` is-${tone}` : ''}`;
  }
  function setWorkspaceStatus(message, tone = '') {
    const element = $('#workspaceDialogStatus');
    element.textContent = message || '';
    element.className = `form-status${tone ? ` is-${tone}` : ''}`;
  }
  ...
  function hideConnectionError() {
    $('#plannerConnectionBanner').classList.add('is-hidden');
  }

  function updateFundingEditor() {
    const type = $('#taskType').value || 'WORK';
    $('#taskFundingSection').classList.toggle('is-hidden', type !== 'FUNDING');
    if (type === 'PURCHASE' && $('#taskOrderStatus').value === 'NOT_NEEDED')
      $('#taskOrderStatus').value = 'NEEDS_SPEC';
  }

  function insightsTasks() {
    return filteredTasks(
      state.insightsScope === 'all'
  ...
  function renderInsights() {
    ...
    if (!tasks.length) {
      host.innerHTML =
        '<div class="insights-empty"><span>◫</span><h3>No work matches this view</h3><p>Choose another project or adjust your filters.</p></div>';
      return;
    }
    const today = todayText();
    ...
    const statusCounts = STATUS.map((item) => ({
      ...item,
      count: tasks.filter((task) => task.status === item.id).length,
    }));
    const priorityCounts = PRIORITY.map((item) => ({
      ...item,
      count: tasks.filter((task) => task.priority === item.id).length,
    }));
    const weeks = dueWeekBuckets(tasks, 8);
    const partCounts = ORDER_STATUS.filter(
      (item) => item.id !== 'NOT_NEEDED',
    ).map((item) => ({
      ...item,
      count: parts.filter((task) => task.orderStatus === item.id).length,
    }));
  ...
  function renderOwnerInsight(owners) {
    ...
    return `<article class="insight-panel"><header><div><span>Capacity</span><h3>Workload by owner</h3></div><small>Top ${Math.min(8, owners.length)}</small></header><div class="owner-load-list">${
      owners.length
        ? owners
            .slice(0, 8)
            .map(
              (owner) =>
                `<div><span class="owner-load-avatar">${escapeHtml(initials(owner.name))}</span><div><strong>${escapeHtml(owner.name)}</strong><small>${owner.active} active${owner.risk ? ` · ${owner.risk} at risk` : ''}</small><div class="bar-track"><i style="width:${(owner.total / max) * 100}%"></i></div></div><b>${owner.total}</b></div>`,
            )
            .join('')
        : '<p class="insight-no-data">Assign owners to see capacity.</p>'
    }</div></article>`;
  }
  ...
  function renderFundingInsight(tasks) {
    ...
    return `<article class="insight-panel insight-wide funding-insight"><header><div><span>Finance</span><h3>Funding opportunity pipeline</h3></div><small>${known ? formatCurrency(known) + ' known ceiling' : 'Variable opportunity values'}</small></header>${
      tasks.length
        ? `<div class="funding-pipeline">${statuses
            .map((group) => {
              const value = group.tasks.reduce(
                (sum, task) => sum + (Number(task.fundingMax) || 0),
                0,
              );
              const scaleValue = value || group.tasks.length;
              return `<div><div class="funding-stage-heading"><span>${group.label}</span><strong>${group.tasks.length}</strong></div><div class="bar-track"><i class="status-bar-${group.id}" style="width:${(scaleValue / max) * 100}%"></i></div><small>${value ? formatCurrency(value) : group.tasks.map(fundingValueLabel).join(' · ')}</small></div>`;
            })
            .join('')}</div>`
        : '<p class="insight-no-data">Mark tasks as Funding opportunity to track Purdue grants and fundraising programs here.</p>'
    }</article>`;
  }
  ...
  function ownerWorkload(tasks) {
    ...
    tasks.forEach((task) => {
      ...
      (names.length ? names : ['Unassigned']).forEach((name) => {
        const item = map.get(normalizeKey(name)) || {
          name,
          total: 0,
          active: 0,
          risk: 0,
  ...
  function listSort(a, b) {
    ...
    const ranks = {
      priority: PRIORITY.map((x) => x.id),
      status: STATUS.map((x) => x.id),
    };
    const value = (task) =>
      field === 'team'
        ? teamForTask(task)?.name || ''
        : field === 'progress'
          ? Number(task.progress || 0)
          : ranks[field]
            ? ranks[field].indexOf(task[field])
            : String(task[field] || '');
    const av = value(a),
      bv = value(b);
    // Unassigned values always follow populated values, in either direction.
    if (av === '' && bv !== '') return 1;
    if (bv === '' && av !== '') return -1;
    const comparison =
      typeof av === 'number'
    ...
    return (
      comparison * (state.sortDirection === 'desc' ? -1 : 1) ||
  ...
  function taskHealth(task) {
    ...
    if (task.status === 'DONE')
      return {
        ...
        label: 'Task health',
        title: 'Complete',
        detail: 'Finished work is reflected in timeline progress.',
      };
    if (
      task.taskType === 'FUNDING' &&
      ['VERIFY_CURRENT', 'CONTACT_REQUIRED'].includes(task.sourceConfidence)
    )
      return {
        tone: 'warning',
        label: 'Funding research',
        title: sourceConfidenceShort(task.sourceConfidence),
        detail:
          'Confirm the current cycle, eligibility, and deadline before treating this as an active application.',
      };
    if (task.status === 'BLOCKED')
      return {
        ...
        label: 'Task health',
        title: 'Blocked',
        detail: 'Name the blocker in a comment and assign the next action.',
      };
    if (task.dueDate && task.dueDate < today)
      return {
        ...
        label: 'Task health',
        title: 'Overdue',
        detail: `Due ${formatShortDate(task.dueDate)}. Update the date, status, or recovery plan.`,
      };
    if (
      task.dueDate &&
      daysBetween(today, task.dueDate) <= 3 &&
      Number(task.progress || 0) < 75
    )
      return {
        tone: 'warning',
        label: 'Task health',
        title: 'At risk',
        detail: 'Due soon with substantial work remaining.',
      };
    if (
      ['NEEDS_SPEC', 'NEEDS_QUOTE', 'READY_TO_ORDER'].includes(task.orderStatus)
    )
      return {
        tone: 'parts',
        label: 'Parts readiness',
        title: orderLabel(task.orderStatus),
        detail: 'Purchasing progress may control the engineering schedule.',
      };
    if (!task.dueDate)
      return {
        tone: 'neutral',
        ...
        title: 'No due date',
        detail:
          'Add dates to make the task visible in the timeline and deadline views.',
      };
    return {
      tone: 'good',
      label: 'Task health',
      title: 'On track',
      detail: `Scheduled through ${formatShortDate(task.dueDate)}.`,
    };
  }
  ...
  function dueState(task) {
    ...
    if (task.status === 'DONE')
      return { text: `Done ${formatShortDate(task.dueDate)}`, tone: 'done' };
    const delta = daysBetween(todayText(), task.dueDate);
    if (delta < 0)
      return { text: `${Math.abs(delta)}d overdue`, tone: 'overdue' };
  ...
  function renderOwnerAvatars(owners) {
    ...
    return (
      owners
        ...
        .map(
          (name) =>
            `<span title="${escapeHtml(name)}">${escapeHtml(initials(name))}</span>`,
        )
        .join('') +
  ...
  function taskTypeLabel(value) {
    return (
      TASK_TYPE.find((item) => item.id === value)?.label || value || 'Work item'
    );
  }
  function statusLabel(value) {
    return (
      STATUS.find((item) => item.id === value)?.label || value || 'Planned'
    );
  }
  function priorityLabel(value) {
    return (
      PRIORITY.find((item) => item.id === value)?.label || value || 'Medium'
    );
  }
  function orderLabel(value) {
    return (
      ORDER_STATUS.find((item) => item.id === value)?.label ||
      value ||
      'Not a purchase'
    );
  }
  function sourceConfidenceLabel(value) {
    return (
      SOURCE_CONFIDENCE.find((item) => item.id === value)?.label ||
      value ||
      'Team-entered information'
    );
  }
  function sourceConfidenceShort(value) {
    const labels = {
      OFFICIAL_CURRENT: 'Official current',
      OFFICIAL_NO_CURRENT_DEADLINE: 'Deadline not posted',
      VERIFY_CURRENT: 'Verify current cycle',
      CONTACT_REQUIRED: 'Contact required',
      TEAM_ENTERED: 'Team entered',
    };
    return labels[value] || 'Team entered';
  }
  function fundingValueLabel(task) {
    if (task.fundingAmountLabel) return task.fundingAmountLabel;
    const min = Number(task.fundingMin) || 0;
    const max = Number(task.fundingMax) || 0;
    if (min && max && min !== max)
  ...
  function isPartsTask(task) {
    return Boolean(
      task.partName ||
        task.partNumber ||
        task.vendor ||
        (task.orderStatus && task.orderStatus !== 'NOT_NEEDED'),
    );
  }
  ...
  function addDaysText(value, count) {
    const date = parseDate(value);
  ...
  function formatBoardDateRange(board) {
    if (board.targetStart && board.targetEnd)
      return `${formatShortDate(board.targetStart)} – ${formatShortDate(board.targetEnd)}`;
    if (board.targetStart)
      return `Starts ${formatShortDate(board.targetStart)}`;
    if (board.targetEnd) return `Target ${formatShortDate(board.targetEnd)}`;
    return '';
  }
