(() => {
  ...
  async function init() {
    ...
    restoreIdentity();
    state.view =
      window.SponsorFlowStorage.getItem('asmeCalendarView') || 'agenda';
    setCalendarView(state.view);
    state.month =
      validDateOnly(window.SponsorFlowStorage.getItem('asmeCalendarMonth')) ||
      state.month;
    state.scope = normalizeStoredScope(
      window.SponsorFlowStorage.getItem('asmeCalendarScope') || 'all',
    );
  ...
  function bindEvents() {
    ...
    $$('button[data-calendar-view]').forEach((button) =>
      button.addEventListener('click', () =>
        setCalendarView(button.dataset.calendarView),
      ),
    );
    $$('[data-calendar-kind]').forEach((button) =>
      button.addEventListener('click', () => {
        state.kind = button.dataset.calendarKind;
        $$('[data-calendar-kind]').forEach((item) => {
    ...
    $('[data-identity-cancel]').addEventListener('click', () =>
      $('#calendarIdentityDialog').close(),
    );
    $('#calendarSaveName').addEventListener('click', () =>
      saveIdentity($('#calendarActorName').value),
    );
    ...
    $('#calendarIdentitySave').addEventListener('click', () => {
      if (saveIdentity($('#calendarIdentityName').value, true))
        $('#calendarIdentityDialog').close();
    });
    $('#calendarIdentityName').addEventListener('keydown', (event) => {
      if (event.key === 'Enter') {
        event.preventDefault();
        $('#calendarIdentitySave').click();
      }
    });
    ...
    $('#calendarNewEventTop')?.addEventListener('click', () =>
      openEventDialog('', { startDate: state.selectedDate || todayText() }),
    );
    $('#calendarSubscribe').addEventListener('click', (event) => {
      event.currentTarget.closest('details')?.removeAttribute('open');
      openSubscriptions();
    });
    $('#calendarSnapshot').addEventListener('click', (event) => {
      event.currentTarget.closest('details')?.removeAttribute('open');
      downloadCurrentCalendar();
    });
    [$('#calendarManage'), $('#calendarManageTop')]
      .filter(Boolean)
      .forEach((button) =>
        button.addEventListener('click', openCalendarManager),
      );
    $('#calendarManagerNew').addEventListener(
      'click',
      resetCalendarManagerForm,
    );
    $('#customCalendarReset').addEventListener(
      'click',
      resetCalendarManagerForm,
    );
    ...
    $$('[data-calendar-manager-close]').forEach((button) =>
      button.addEventListener('click', () =>
        $('#calendarManagerDialog').close(),
      ),
    );
    ...
    $('#calendarEventStartDate').addEventListener('change', () => {
      ...
      if (start && (!end || end < start))
        $('#calendarEventEndDate').value = start;
      updateHealth();
      markDirty();
    });
    $('#calendarEventEndDate').addEventListener('change', () => {
      updateHealth();
      markDirty();
    });
    [
    ...
    $('#calendarEventDownload').addEventListener('click', () => {
      ...
      if (task.startDate || task.dueDate)
        downloadIcs([task], task.title || 'ASME event');
    });
    $$('[data-calendar-close]').forEach((button) =>
      button.addEventListener('click', closeEventDialog),
    );
    $$('[data-subscription-close]').forEach((button) =>
      button.addEventListener('click', () =>
        $('#calendarSubscriptionsDialog').close(),
      ),
    );
    [
      $('#calendarIdentityDialog'),
      $('#calendarEventDialog'),
      $('#calendarSubscriptionsDialog'),
      $('#calendarManagerDialog'),
    ].forEach((dialog) => {
      dialog.addEventListener('click', (event) => {
        if (event.target !== dialog) return;
        if (dialog.id === 'calendarEventDialog') closeEventDialog();
        else dialog.close();
      });
      dialog.addEventListener('cancel', (event) => {
        if (dialog.id === 'calendarEventDialog' && state.dirty) {
          event.preventDefault();
          closeEventDialog();
        }
      });
  ...
  async function loadCalendar() {
    ...
    if (snapshot && !state.tasks.length) applyCalendarSnapshot(snapshot.data);
    setLoading(!snapshot && !state.tasks.length);
    $('#calendarSyncStatus').textContent = snapshot
      ? 'Showing saved dates · checking for updates…'
      : 'Connecting to the club calendar…';
    try {
      const data = await API.post('plannerBootstrap');
      applyCalendarSnapshot(data);
      ...
      if (queryTask && state.tasks.some((task) => task.id === queryTask))
        setTimeout(() => openEventDialog(queryTask), 120);
    } catch (error) {
      $('#calendarSyncStatus').textContent = snapshot
        ? 'Saved dates · updates unavailable'
        : 'Could not connect';
  ...
  function renderScopeOptions() {
    ...
    $('#calendarScopeSelect').innerHTML =
      ...
      state.boards
        .map(
          (board) =>
            `<option value="board:${escapeHtml(board.id)}">${escapeHtml(board.name)}</option>`,
        )
        .join('');
    state.scope = normalizeScopeAgainstData(state.scope);
    $('#calendarScopeSelect').value = state.scope;
  ...
  function normalizeScopeAgainstData(value) {
    ...
    if (scope === 'all' || scope === 'important' || scope === 'general')
      return scope;
    ...
    if (scope.startsWith('custom:')) {
      return state.calendars.some((calendar) => calendar.id === scope.slice(7))
        ? scope
        : 'all';
    }
    return 'all';
    ...
    const grouped = state.teams
      .map((team) => {
        ...
        return boards.length
          ? `<optgroup label="${escapeHtml(team.name)}">${boards.map((board) => `<option value="${escapeHtml(board.id)}">${escapeHtml(board.name)}</option>`).join('')}</optgroup>`
          : '';
      })
      .join('');
    $('#calendarEventBoard').innerHTML =
      grouped || '<option value="">No timelines available</option>';
    const target =
  ...
  function defaultBoardForScope() {
    ...
    if (state.scope === 'general') {
      return (
        state.boards.find((board) => board.teamId === GENERAL_TEAM_ID)?.id ||
        state.boards.find((board) => board.id === CLUB_BOARD_ID)?.id ||
        state.boards[0]?.id ||
        ''
      );
    }
    if (state.scope.startsWith('team:')) {
      return (
        state.boards.find((board) => board.teamId === state.scope.slice(5))
          ?.id ||
        state.boards.find((board) => board.id === CLUB_BOARD_ID)?.id ||
        state.boards[0]?.id ||
        ''
    ...
    if (state.scope.startsWith('custom:')) {
      const calendar = state.calendars.find(
        (item) => item.id === state.scope.slice(7),
      );
      const firstTeamId =
        calendar?.teamIds?.[0] ||
        (calendar?.includeGeneral ? GENERAL_TEAM_ID : '');
      return (
        state.boards.find((board) => board.teamId === firstTeamId)?.id ||
        state.boards.find((board) => board.id === CLUB_BOARD_ID)?.id ||
        state.boards[0]?.id ||
        ''
      );
  ...
  function scopeMeta() {
    if (state.scope.startsWith('board:')) {
      const board = state.boards.find(
        (item) => item.id === state.scope.slice(6),
      );
      return {
        name: board?.name || 'Project',
        description:
          board?.description || "This project's dates and deadlines.",
      };
    }
    if (state.scope === 'important') {
      return {
        name: 'Key dates',
        description:
          'Meetings, milestones, and critical dates. Funding deadlines are listed with their projects.',
      };
    }
    if (state.scope === 'general') {
      ...
      return {
        ...
        description:
          team?.description ||
          'Shared club-level milestones, cross-team deadlines, and general planning.',
      };
    }
    if (state.scope.startsWith('team:')) {
      ...
      return {
        ...
        description:
          team?.description || 'Every dated item owned by this subteam.',
      };
    }
    if (state.scope.startsWith('custom:')) {
      const calendar = state.calendars.find(
      ...
      return {
        ...
        description:
          ...
          'A member-created combination of team calendars.',
    ...
    return {
      name: 'All dates',
      description:
        'Events and deadlines across active teams. Finance records are available in the sidebar.',
    };
  }
  ...
  function tasksForScope() {
    ...
    if (state.scope.startsWith('board:'))
      return tasks.filter((task) => task.boardId === state.scope.slice(6));
    if (state.scope === 'important')
      return tasks.filter(
        (task) =>
          isImportantTask(task) &&
          state.boards.find((board) => board.id === task.boardId)?.teamId !==
            'TEAM-OPS',
      );
    if (state.scope === 'general') {
      const boardIds = new Set(
        state.boards
          .filter((board) => board.teamId === GENERAL_TEAM_ID)
          .map((board) => board.id),
      );
    ...
    if (state.scope.startsWith('team:')) {
      const boardIds = new Set(
        state.boards
          .filter((board) => board.teamId === state.scope.slice(5))
          .map((board) => board.id),
      );
      return tasks.filter((task) => boardIds.has(task.boardId));
    }
    if (state.scope.startsWith('custom:')) {
      ...
      const boardIds = new Set(
        state.boards
          .filter((board) => teamIds.has(board.teamId))
          .map((board) => board.id),
      );
      const selected = tasks.filter((task) => boardIds.has(task.boardId));
      return calendar.importantOnly
        ? selected.filter(isImportantTask)
        : selected;
    }
    return tasks.filter(
      (task) =>
  ...
  function renderCalendar() {
    ...
    if (
      state.selectedDate &&
      !state.selectedDate.startsWith(state.month.slice(0, 7))
    )
      state.selectedDate = '';

    const tasks = visibleCalendarTasks().sort(
      (a, b) =>
        (taskBounds(a).start || '').localeCompare(taskBounds(b).start || '') ||
        a.title.localeCompare(b.title),
    );
    const cells = [];
    for (let index = 0; state.view === 'month' && index < 42; index += 1) {
      ...
      const eventButtons = dayTasks
        .slice(0, 4)
        .map((task, eventIndex) => calendarEventHtml(task, value, eventIndex))
        .join('');
      const desktopMore =
        dayTasks.length > 4
          ? `<button class="calendar-more-button" type="button" data-focus-date="${value}">+${dayTasks.length - 4} more</button>`
          : '';
      const mobileMore =
        dayTasks.length > 2
          ? `<button class="calendar-mobile-count" type="button" data-focus-date="${value}" aria-label="Show all ${dayTasks.length} items on ${value}">+${dayTasks.length - 2}</button>`
          : '';
    ...
    $('#calendarGrid')
      .querySelectorAll('[data-focus-date]')
      .forEach((button) =>
        button.addEventListener('click', (event) => {
          event.stopPropagation();
    ...
    $('#calendarGrid')
      .querySelectorAll('[data-select-date]')
      .forEach((cell) => {
    ...
    const monthTasks = tasks.filter((task) =>
      intersects(task, startText, endText),
    );
    ...
    $('#calendarAgenda')
      .querySelectorAll('[data-agenda-task]')
      .forEach((button) =>
        button.addEventListener('click', () =>
          openEventDialog(button.dataset.agendaTask),
        ),
      );
    $('#calendarAgenda')
      .querySelectorAll('[data-agenda-download]')
      .forEach((button) =>
        button.addEventListener('click', (event) => {
          event.stopPropagation();
          const task = state.tasks.find(
            (item) => item.id === button.dataset.agendaDownload,
          );
  ...
  function visibleCalendarTasks() {
    ...
    return tasksForScope().filter((task) => {
      ...
      if (state.kind === 'deadlines' && task.taskType === 'MEETING')
        return false;
      return (
        !search ||
        [task.title, task.description, task.ownerNames, task.location]
          .join(' ')
          .toLowerCase()
          .includes(search)
      );
  ...
  function renderCalendarNavigation() {
    ...
    const item = (scope, name) =>
      `<button type="button" class="sidebar-item${state.scope === scope ? ' is-active' : ''}" data-calendar-scope="${escapeHtml(scope)}"${state.scope === scope ? ' aria-current="true"' : ''}><span>${escapeHtml(name)}</span></button>`;
    host.innerHTML =
      item('all', 'All dates') +
      item('important', 'Key dates') +
      item('general', 'Club calendar') +
      '<p class="sidebar-label">Teams & projects</p>' +
      calendarTeams()
        .filter((team) => team.id !== 'TEAM-OPS')
        .map(
          (team) =>
            `<details class="sidebar-group" data-team="${escapeHtml(team.id)}"${closed.has(team.id) ? '' : ' open'}><summary>${escapeHtml(team.name)}</summary>${item('team:' + team.id, 'All team dates')}${state.boards
              .filter((board) => board.teamId === team.id)
              .map((board) => item('board:' + board.id, board.name))
              .join('')}</details>`,
        )
        .join('') +
      ...
      calendarTeams()
        .filter((team) => team.id === 'TEAM-OPS')
        .map((team) => item('team:' + team.id, team.name))
        .join('') +
      '</details>' +
      (state.calendars.length
        ? '<p class="sidebar-label">Custom calendars</p>' +
          state.calendars
            .map((calendar) => item('custom:' + calendar.id, calendar.name))
            .join('')
        : '');
  }
  ...
  function updateAgendaHeading(count) {
    ...
    if (selected) {
      ...
      $('#calendarAgendaTitle').textContent = date.toLocaleDateString('en-US', {
        weekday: 'long',
        month: 'short',
        day: 'numeric',
      });
  ...
  function calendarEventHtml(task, value, eventIndex = 0) {
    ...
    const time =
      task.allDay === false && task.startTime && marker !== 'due'
  ...
  function compactEventLabel(title) {
    const words = String(title || 'Event')
      .trim()
      .split(/\s+/)
      .filter(Boolean);
  ...
  function focusAgenda(value) {
    ...
    const item = $('#calendarAgenda').querySelector(
      `[data-agenda-date="${CSS.escape(value)}"]`,
    );
    if (!item) return;
  ...
  function openEventDialog(taskId = '', defaults = {}) {
    if (!state.actorName) {
      state.afterIdentity = () => openEventDialog(taskId, defaults);
      requireActor();
      return;
    }
    resetEventForm();
    ...
    if (task) {
      ...
      $('#calendarEventDialogTitle').textContent =
        task.taskType === 'MEETING' ? 'Edit event' : 'Edit calendar item';
      $('#calendarEventEyebrow').textContent =
        task.taskType === 'MEETING' ? 'Calendar event' : 'Project deadline';
      ...
      $('#calendarEventEndDate').value =
        validDateOnly(defaults.endDate) || start;
    ...
    fullTaskLink.href = task
      ? `planner.html?board=${encodeURIComponent(task.boardId)}&task=${encodeURIComponent(task.id)}`
      : 'planner.html';
  ...
  function currentFormTask() {
    ...
    const start = validDateOnly($('#calendarEventStartDate').value);
    const end =
      validDateOnly($('#calendarEventEndDate').value) ||
      (state.editingTask ? '' : start);
  ...
  async function saveEvent(event) {
    ...
    if (task.title.length < 2)
      return setEventStatus('Give this item a clear title.', 'error');
    if (!task.boardId) return setEventStatus('Choose a project.', 'error');
    if (!task.startDate && !task.dueDate)
      return setEventStatus('Choose a start or end date.', 'error');
    if (
      !Number.isFinite(task.progress) ||
      task.progress < 0 ||
      task.progress > 100
    )
      return setEventStatus('Progress must be between 0 and 100.', 'error');
    ...
    try {
      ...
      state.calendarFeedBaseUrl = String(
        data.calendarFeedBaseUrl || state.calendarFeedBaseUrl,
      );
      const verified = state.tasks.find((item) => item.id === saved.id);
      if (
        !verified ||
        verified.startDate !== task.startDate ||
        verified.dueDate !== task.dueDate
      )
        throw new Error(
          'The event was saved, but its dates did not verify correctly. Refresh and try once more.',
        );
      state.month = `${(verified.startDate || verified.dueDate).slice(0, 7)}-01`;
      state.selectedDate = isCompactCalendar()
        ? verified.startDate || verified.dueDate
        : '';
      window.SponsorFlowStorage.setItem('asmeCalendarMonth', state.month);
      state.dirty = false;
      ...
      renderCalendar();
      setPageMessage(
        `${verified.title} saved on ${formatShortDate(verified.startDate || verified.dueDate)}.`,
        'success',
      );
    } catch (error) {
  ...
  function updateHealth() {
    ...
    if (!task.startDate && !task.dueDate) {
      ...
      host.innerHTML =
        '<span>Schedule health</span><strong>Add a date</strong><p>Choose dates to place this item on the shared calendar.</p>';
      return;
    }
    ...
    let detail = task.allDay
      ? 'All-day calendar item.'
      : `${formatClock(task.startTime || '09:00')} – ${formatClock(task.endTime || '10:00')}`;
    if (task.status !== 'DONE' && task.dueDate && task.dueDate < today) {
      tone = 'overdue';
      title = 'Past due';
      detail = `Ended ${formatShortDate(task.dueDate)}.`;
    } else if (task.status === 'BLOCKED') {
      tone = 'blocked';
      title = 'Blocked';
      detail = 'Resolve the blocker before this date.';
    }
  ...
  function openSubscriptions() {
    if (!state.calendarFeedBaseUrl)
      return setPageMessage(
        'The live calendar backend is unavailable. Redeploy the current Apps Script version.',
        'error',
      );
    const feeds = [
      ...(state.scope.startsWith('board:')
        ? [
            {
              name: scopeMeta().name,
              description: 'Dates for this project.',
              url: feedUrl('board', state.scope.slice(6)),
            },
          ]
        : []),
      {
        name: 'All dates',
        description: 'Every dated item across the entire workspace.',
        url: feedUrl('club'),
      },
      {
        name: 'Key dates',
        description:
          'Meetings, milestones, and critical dates. Funding deadlines are listed with their projects.',
        url: feedUrl('important'),
      },
      {
        name: 'Club calendar',
        description: 'Shared club-level planning and cross-team milestones.',
        url: feedUrl('team', GENERAL_TEAM_ID),
      },
      ...calendarTeams().map((team) => ({
        name: team.name,
        description: team.description || "This subteam's dated work.",
        url: feedUrl('team', team.id),
      })),
      ...state.calendars.map((calendar) => ({
        name: calendar.name,
        description: calendar.description || 'Member-created calendar.',
        url: feedUrl('custom', calendar.id),
        custom: true,
        color: calendar.color,
      })),
    ];
    $('#calendarSubscriptionList').innerHTML = feeds
      .map(
        (
          feed,
        ) => `<article class="calendar-subscription-card${feed.custom ? ' is-custom' : ''}">
    ...
    $('#calendarSubscriptionList')
      .querySelectorAll('[data-copy-feed]')
      .forEach((button) =>
        button.addEventListener('click', () =>
          copyText(button.dataset.copyFeed),
        ),
      );
  ...
  function feedUrl(scope, id = '') {
    ...
    url.searchParams.set(
      'app',
      new URL('calendar.html', window.location.href).href,
    );
    return url.toString();
  }
  ...
  function downloadCurrentCalendar() {
    ...
    if (!tasks.length)
      return setPageMessage(
        'This calendar has no dated items to download.',
        'error',
      );
    downloadIcs(tasks, scopeMeta().name);
    setPageMessage(
      `${tasks.length} calendar ${tasks.length === 1 ? 'item' : 'items'} downloaded.`,
      'success',
    );
  }
  ...
  function downloadIcs(tasks, name) {
    ...
    const body = [
      'BEGIN:VCALENDAR',
      'VERSION:2.0',
      'PRODID:-//Purdue Indianapolis ASME//ASME Indy Workspace 1.3//EN',
      'CALSCALE:GREGORIAN',
      'METHOD:PUBLISH',
      `X-WR-CALNAME:${icsEscape(name || 'ASME Indy Calendar')}`,
      events,
      'END:VCALENDAR',
      '',
    ].join('\r\n');
    downloadFile(
      `${slugify(name || 'asme-indy-calendar')}.ics`,
      body,
  ...
  function taskIcs(task) {
    ...
    const lines = [
      'BEGIN:VEVENT',
      `UID:${icsEscape(task.id || crypto.randomUUID())}@asmeindy.purdue.edu`,
      `DTSTAMP:${icsTimestamp(new Date())}`,
      `LAST-MODIFIED:${icsTimestamp(new Date(task.updatedAt || Date.now()))}`,
    ];
    if (task.allDay === false && task.startTime) {
      lines.push(
        `DTSTART;TZID=America/Indiana/Indianapolis:${icsDateTime(bounds.start, task.startTime)}`,
      );
      lines.push(
        `DTEND;TZID=America/Indiana/Indianapolis:${icsDateTime(bounds.end, task.endTime || addMinutes(task.startTime, 60))}`,
      );
    } else {
      lines.push(`DTSTART;VALUE=DATE:${bounds.start.replaceAll('-', '')}`);
      lines.push(
        `DTEND;VALUE=DATE:${addDays(bounds.end, 1).replaceAll('-', '')}`,
      );
    }
    lines.push(
      `SUMMARY:${icsEscape(task.title || 'ASME item')}`,
  ...
  function renderCalendarManagerTeams() {
    $('#customCalendarTeams').innerHTML =
      calendarTeams()
        .map(
          (team) =>
            `<label class="calendar-team-option"><input type="checkbox" value="${escapeHtml(team.id)}"><span><b>${escapeHtml(team.name)}</b><small>${escapeHtml(team.description || "Include this team's dated work")}</small></span></label>`,
        )
        .join('') ||
      '<p class="calendar-manager-empty">No active subteams are available.</p>';
  }

  function renderCustomCalendarList() {
    $('#customCalendarCount').textContent = String(state.calendars.length);
    $('#customCalendarList').innerHTML = state.calendars.length
      ? state.calendars
          .map((calendar) => {
            const teamNames = (calendar.teamIds || [])
        ...
        <span class="custom-calendar-swatch"></span><span><strong>${escapeHtml(calendar.name)}</strong><small>${escapeHtml(calendar.description || teamNames.join(' · ') || 'Custom calendar')}</small><em>${escapeHtml(teamNames.join(' · ') || 'No sources')}${calendar.importantOnly ? ' · Important only' : ''}</em></span><b aria-hidden="true">→</b>
          ...
          .join('')
      : '<div class="calendar-manager-empty"><span>⌁</span><strong>No custom calendars yet</strong><p>Create one to combine the subteams and dates that matter to a specific group.</p></div>';
    $('#customCalendarList')
      .querySelectorAll('[data-edit-calendar]')
      .forEach((button) =>
        button.addEventListener('click', () =>
          editCustomCalendar(button.dataset.editCalendar),
        ),
      );
  }
  ...
  async function saveCustomCalendar(event) {
    ...
    const teamIds = $('#customCalendarTeams').querySelectorAll(
    ...
    try {
      ...
      if (index === -1) state.calendars.push(saved);
      else state.calendars[index] = saved;
      ...
      setCalendarManagerStatus(
        `${saved.name} is ready to view and subscribe to.`,
        'success',
      );
  ...
  async function archiveCustomCalendar() {
    ...
    if (
      !window.confirm(
        `Remove “${calendar.name}”? Tasks and events will stay in their original team timelines.`,
      )
    )
      return;
    ...
    try {
      await API.post('archivePlannerCalendar', {
        id,
        actorName: state.actorName,
      });
      ...
      resetCalendarManagerForm();
      setCalendarManagerStatus(
        'Calendar removed. Its tasks and events were not changed.',
        'success',
      );
    } catch (error) {
      setCalendarManagerStatus(error.message, 'error');
  ...
  function updateIdentityUi() {
    ...
    status.textContent = state.actorName
      ? `Editing as ${state.actorName}`
      : 'Required to edit';
  ...
  function requireActor() {
    ...
    $('#calendarIdentityStatus').textContent =
  ...
  function addMinutes(value, amount) {
    const [h, m] = validTime(value).split(':').map(Number);
    const total = (h * 60 + m + amount) % 1440;
  ...
  function relativeTime(value) {
    ...
    if (seconds < 3600)
      return format.format(-Math.floor(seconds / 60), 'minute');
    if (seconds < 86400)
  ...
  function setLoading(active) {
    $('#calendarLoading').classList.toggle('is-hidden', !active);
  }
  ...
  function setPageMessage(message, tone = '') {
    const host = $('#calendarMessage');
    host.textContent = message || '';
    host.className = `form-status calendar-page-message${tone ? ` is-${tone}` : ''}`;
  }
  function setEventStatus(message, tone = '') {
    const host = $('#calendarEventFormStatus');
    host.textContent = message || '';
    host.className = `form-status event-form-status${tone ? ` is-${tone}` : ''}`;
  }
  function setSubscriptionStatus(message, tone = '') {
    const host = $('#calendarSubscriptionStatus');
    host.textContent = message || '';
    host.className = `form-status${tone ? ` is-${tone}` : ''}`;
  }
