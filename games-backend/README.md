...
**Already installed Games?** Replace only Games.gs, run `setupGames` once to append the timed-scoring columns, and deploy a new version of the existing web app. Keep your existing Code.gs routing hooks and Admin.html. See [RELEASE-5.2.md](../RELEASE-5.2.md) for the full update.
...
8. Verify on two devices: join the leaderboard on one, finish a game, and confirm its score appears on the other. Enter the same name on both devices to automatically reconnect to the same player; no code is needed. Also open attendance, the officer dashboard, and a calendar subscription to confirm the existing routes still work.
...
The previous nine-column Games Results schema is upgraded by appending six columns. Extra columns and reordered columns are preserved. Missing core identity/result headers or duplicated known headers stop setup before editing either tab. It never deletes tabs or creates another spreadsheet. Correctly initialized Games tabs keep their rows when setup is rerun.

## What gets stored

- **Games Players**: a player ID, display name, normalized name, and timestamps. Historical codeHash cells are preserved; new clients identify by name only.
...
- Display names and rankings are public on the Games page. Hashes are never returned in standings.
...
- The same normalized name restores identity and completed results on another device. Changing names switches players. Unfinished boards stay on their original device. Offline results can sync for 35 days; the spreadsheet retains historical scores.
...
[Games.gs](Games.gs) is generated from `assets/games/core.js`, `assets/games/metrics.js`, and `games-backend/service.gs`:
...
Rebuild after changing any source, replace only Games.gs in the existing project, and deploy a new version of the existing web app. Version 1 puzzle rules and seeds must remain stable after launch; introduce a new challenge version for future changes that affect saved results.
