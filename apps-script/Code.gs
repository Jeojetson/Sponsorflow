...
function upgradeSponsorFlowV11() {
  ...
  const plannerResult = ensureDefaultPlannerStructureSafe_();
  SpreadsheetApp.getUi().alert(
    `SponsorFlow 1.1 is ready. ${normalized} existing planner date cells were normalized to stable YYYY-MM-DD text values. ` +
    `Existing sponsors, templates, teams, timelines, tasks, owners, progress, comments, and funding research were preserved. ` +
    `${plannerResult.teamsCreated} missing teams, ${plannerResult.boardsCreated} missing timelines, and ${plannerResult.tasksCreated} missing starter items were added.`
  );
}
...
function plannerBoardPublic_(row) {
  return {
    ...
    name: row.name,
    description: row.description || '',
    targetStart: plannerDateOnly_(row.targetStart),
    targetEnd: plannerDateOnly_(row.targetEnd),
    active: toBool_(row.active),
    createdBy: row.createdBy || '',
    updatedBy: row.updatedBy || '',
    createdAt: row.createdAt || '',
...
function plannerTaskPublic_(row, commentCount) {
  return {
    ...
    startDate: plannerDateOnly_(row.startDate),
    dueDate: plannerDateOnly_(row.dueDate),
...
function readObjects_(name) {
  ...
  return values.slice(1).filter(row => row.some(value => value !== '')).map(row => {
    const object = {};
    headers.forEach((header, index) => object[header] = decodeCellForHeader_(header, row[index]));
    return object;
  });
}
...
function appendObject_(name, object) {
  ...
  const values = headers.map(header => encodeCellForHeader_(header, object[header] == null ? '' : object[header]));
...
function updateObjectById_(name, id, updates) {
  ...
  headers.forEach((header, columnIndex) => {
    if (PLANNER_DATE_ONLY_HEADERS_.indexOf(header) !== -1) {
      values[rowIndex][columnIndex] = encodeCellForHeader_(header, values[rowIndex][columnIndex]);
    }
  });
  Object.keys(updates).forEach(key => {
    const columnIndex = headers.indexOf(key);
    if (columnIndex >= 0) values[rowIndex][columnIndex] = encodeCellForHeader_(key, updates[key]);
  });
...
const PLANNER_DATE_ONLY_HEADERS_ = ['startDate', 'dueDate', 'targetStart', 'targetEnd'];

function plannerDateOnly_(value) {
  if (value instanceof Date) return Utilities.formatDate(value, Session.getScriptTimeZone() || 'America/Indiana/Indianapolis', 'yyyy-MM-dd');
  const text = String(value == null ? '' : value).trim();
  const match = text.match(/^(\d{4})-(\d{2})-(\d{2})/);
  return match ? `${match[1]}-${match[2]}-${match[3]}` : '';
}

function decodeCellForHeader_(header, value) {
  if (PLANNER_DATE_ONLY_HEADERS_.indexOf(header) !== -1) return plannerDateOnly_(value);
  return decodeCell_(value);
}

function encodeCellForHeader_(header, value) {
  if (PLANNER_DATE_ONLY_HEADERS_.indexOf(header) !== -1) return plannerDateOnly_(value);
  return encodeCell_(value);
}

function normalizePlannerDateStorage_() {
  let changed = 0;
  [SF.SHEETS.PLANNER_TASKS, SF.SHEETS.PLANNER_BOARDS].forEach(name => {
    const sheet = sheet_(name);
    const range = sheet.getDataRange();
    const values = range.getValues();
    if (values.length < 2) return;
    const headers = values[0].map(String);
    const dateColumns = headers.map((header, index) => PLANNER_DATE_ONLY_HEADERS_.indexOf(header) !== -1 ? index : -1).filter(index => index >= 0);
    if (!dateColumns.length) return;
    for (let row = 1; row < values.length; row += 1) {
      dateColumns.forEach(column => {
        const original = values[row][column];
        const normalized = plannerDateOnly_(original);
        if (String(original == null ? '' : original) !== normalized || original instanceof Date) {
          values[row][column] = normalized;
          changed += 1;
        }
      });
    }
    dateColumns.forEach(column => sheet.getRange(2, column + 1, Math.max(values.length - 1, 1), 1).setNumberFormat('@'));
